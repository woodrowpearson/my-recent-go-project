...
func main() {
	...
	flag.Parse()
	receivedOut, err := os.Create("logs/received.log")
	check(err)
	defer receivedOut.Close()
	swapOut, err := os.Create("logs/swapped.log")
	check(err)
	defer swapOut.Close()

	courierOut, err := os.Create("logs/courier_out.log")
	check(err)
	defer courierOut.Close()
	courierErr, err := os.Create("logs/courier_err.log")
	check(err)
	defer courierOut.Close()
	dispatchOut, err := os.Create("logs/dispatch_out.log")
	check(err)
	defer dispatchOut.Close()
	dispatchErr, err := os.Create("logs/dispatch_err.log")
	check(err)
	defer courierOut.Close()
